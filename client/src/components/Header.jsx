...
const Header = () => {
  ...
  return (
    <header
      className={`fixed inset-x-0 px-4 lg:px-32 xl:px-44 py-4 flex items-center justify-between z-50 transition-all duration-300 ${
        color ? "bg-white" : "bg-transparent"
      }`}
    >
      {/* image section */}
      <NavLink to={"/"}>
        <p className="text-3xl font-bold font-poppins h-auto object-contain">freepic</p>
        {/* <img
          src={Logo}
          alt="Main Logo"
      ...
      <div className="flex items-center justify-center gap-4">
        ...
        {user && (
          <Link
            to={"/newPost/upload"}
            className="px-4 py-2 rounded-full text-base font-semibold text-white  cursor-pointer bg-emerald-500 hover:bg-emerald-400 duration-150" 
          >
            Upload
          </Link>
        )}
